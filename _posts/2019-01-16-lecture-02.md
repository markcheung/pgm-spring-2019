...
* Example: $$ X \perp Y | Z$$, then we say $Z$$ D-separates $$X$$ and $$Y$$ then

<img src="{{ '/assets/img/notes/lecture-02/graph_separation_criterion_example.png' | relative_url }}" width="500" />

  * Ancestral graph (focusing only the nodes of interest): the nodes themselves + ancestors of the nodes in question.

  * Moral ancestral: remove the connectivity between nodes, but connect parents to nodes that are shared by a single child.

If there is a way to travel from one node to another node using any path, then these two nodes are not independent (example is not conditionally independent).
