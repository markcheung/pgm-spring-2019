...
If there is a way to travel from one node to another node using any path (not thru the given), then these two nodes are not independent (example is not conditionally independent).
